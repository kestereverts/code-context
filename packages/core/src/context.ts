...
import { GeminiContextualizer, ContextualizeItem } from "./enrichment";
import {
  VectorDatabase,
  VectorDocument,
  VectorSearchResult,
...
const DEFAULT_SUPPORTED_EXTENSIONS = [
  ...
  ".html",
  ".htm", // HTML (Angular/Vue templates, static pages)
  ".css", // CSS
  ".scss", // SCSS
  ".json", // JSON (configs, schemas)
  ".yaml",
  ".yml", // YAML (CI/CD, k8s, docker-compose)
...
// Progress bands (percent) used when the batch contextualization pre-pass runs.
// The opaque async batch wait gets the largest band and is advanced by an
// elapsed-time creep, so progress moves smoothly instead of freezing then
// jumping to 100% once embedding starts.
const BATCH_SPLIT_BAND = { start: 5, end: 15 }; // reading + splitting files
const BATCH_WAIT_BAND = { start: 15, end: 75 }; // waiting on the async job(s)
const BATCH_EMBED_START = 75; // streaming embed/insert begins here
const BATCH_CREEP_TIME_CONSTANT_MS = 180_000; // ~3 min to fill ~63% of a job's sub-band
...
export interface ContextConfig {
  ...
  contextualizer?: GeminiContextualizer; // Optional: enrich chunks with LLM-generated context before embedding
  useBatchContextualization?: boolean; // Optional: use the async Gemini Batch API for contextualization
}
...
export class Context {
  ...
  constructor(config: ContextConfig = {}) {
    ...
    // Optional contextual-retrieval enrichment (off unless explicitly enabled).
    this.contextualizer =
      config.contextualizer || this.createContextualizerFromEnv();
    if (config.useBatchContextualization !== undefined) {
      this.useBatchContextualization = config.useBatchContextualization;
    }
  ...
  /**
   * Build a contextual-retrieval enricher from environment variables.
   *
   * Gated behind CONTEXTUAL_RETRIEVAL_ENABLED=true and a GEMINI_API_KEY.
   * Currently Gemini-only; the LLM model defaults to gemini-3.1-flash-lite
   * and can be overridden with CONTEXTUAL_RETRIEVAL_MODEL.
   */
  private createContextualizerFromEnv(): GeminiContextualizer | undefined {
    const enabled =
      (envManager.get("CONTEXTUAL_RETRIEVAL_ENABLED") || "").toLowerCase() ===
      "true";
    if (!enabled) {
      return undefined;
    }
    ...
    if (!apiKey) {
      console.warn(
        "[Context] ⚠️  CONTEXTUAL_RETRIEVAL_ENABLED is set but GEMINI_API_KEY is missing; contextual retrieval disabled.",
      );
      return undefined;
    }

    const model =
      envManager.get("CONTEXTUAL_RETRIEVAL_MODEL") || "gemini-2.5-flash-lite";
    const baseURL = envManager.get("GEMINI_BASE_URL");

    // Batch API: ~50% cheaper, async. Runs as a single blocking pre-pass.
    this.useBatchContextualization =
      (envManager.get("CONTEXTUAL_RETRIEVAL_BATCH") || "").toLowerCase() ===
      "true";
    const pollMs = this.parsePositiveInt(
      envManager.get("CONTEXTUAL_RETRIEVAL_BATCH_POLL_MS"),
    );
    const timeoutMs = this.parsePositiveInt(
      envManager.get("CONTEXTUAL_RETRIEVAL_BATCH_TIMEOUT_MS"),
    );

    console.log(
      `[Context] 🧩 Contextual retrieval ENABLED (Gemini model: ${model}${this.useBatchContextualization ? ", Batch API" : ""})`,
    );
    return new GeminiContextualizer({
      apiKey,
      model,
      ...(baseURL && { baseURL }),
      ...(pollMs !== undefined && { batchPollIntervalMs: pollMs }),
      ...(timeoutMs !== undefined && { batchMaxWaitMs: timeoutMs }),
    });
  }
  ...
  public getCollectionName(codebasePath: string): string {
    ...
    const configOverride = this.getValidOverrideValue(
      this.collectionNameOverride,
    );
    if (configOverride) {
      const suffix = this.sanitizeCollectionNameSuffix(
        configOverride,
        prefix,
        pathHash,
        "Context config",
    ...
    const envOverride = this.getValidOverrideValue(
      envManager.get("CODE_CHUNKS_COLLECTION_NAME_OVERRIDE"),
    );
    if (envOverride) {
      const suffix = this.sanitizeCollectionNameSuffix(
        envOverride,
  ...
  private sanitizeCollectionNameSuffix(
    ...
    if (sanitized !== normalized) {
      ...
      if (!this.warnedOverrideSanitization.has(warningKey)) {
        console.warn(
          `[Context] ⚠️ Sanitized collection name override from "${normalized}" to "${sanitized}" (${source}); final suffix "${full}"`,
        );
  ...
  async indexCodebase(
    ...
    const supportedExtensions = this.getEffectiveSupportedExtensions(
    ...
    // Optional contextual-retrieval pre-pass: generate every chunk's context
    ...
    if (usedBatchPrepass) {
      try {
        ...
        if (error instanceof IndexAbortError) throw error;
        console.warn(
    ...
    // When the batch pre-pass ran it already consumed the lower bands, so the
    // embed/insert pass takes the top slice; otherwise it owns 10%→100%.
    const indexingStartPercentage = usedBatchPrepass ? BATCH_EMBED_START : 10;
  ...
  async semanticSearch(
    ...
    if (isHybrid === true) {
      ...
      console.log(
        `[Context] ✅ Found ${results.length} results, ${dedupedResults.length} after dedup`,
      );
  ...
  private async processFileList(
    ...
    for (let i = 0; i < filePaths.length; i++) {
      ...
      if (signal?.aborted) {
        throw new IndexAbortError(
          `Indexing aborted after processing ${processedFiles}/${filePaths.length} files`,
        );
      }
   ...
   */
  /**
   * Generate per-chunk context blurbs for contextual retrieval. Returns an
   * array aligned with `chunks` (empty string where disabled or on failure).
   * Enclosing-file reads are cached per batch to avoid re-reading.
   */
  /**
   * Batch-contextualization pre-pass: split every file, generate all context
   * blurbs in one (or few) async Gemini Batch API job(s), and cache them keyed
   * by the same id used at insert time. Runs before the streaming embed/insert
   * pass so the per-buffer path becomes a cheap map lookup.
   */
  private async precomputeBatchContexts(
    codeFiles: string[],
    codebasePath: string,
    splitter: Splitter,
    progressCallback?: (progress: {
    ...
    for (let fi = 0; fi < totalFiles; fi++) {
      const filePath = codeFiles[fi];
      if (signal?.aborted) {
        throw new IndexAbortError(
          "Indexing aborted during contextual-retrieval pre-pass",
        );
      }

      // Smoothly report the read+split sub-phase across its band.
      const splitPct = Math.round(
        BATCH_SPLIT_BAND.start +
          (fi / totalFiles) * (BATCH_SPLIT_BAND.end - BATCH_SPLIT_BAND.start),
      );
      if (splitPct !== lastSplitPct) {
        lastSplitPct = splitPct;
        progressCallback?.({
          phase: `Contextual retrieval: scanning files (${fi + 1}/${totalFiles})...`,
          current: fi + 1,
          total: totalFiles,
          percentage: splitPct,
        });
      ...
      try {
        ...
        console.warn(
          `[Context] ⚠️  Batch pre-pass skipping unreadable file ${filePath}: ${error}`,
        );
        continue;
      }
      ...
      try {
        ...
        console.warn(
          `[Context] ⚠️  Batch pre-pass failed to split ${filePath}: ${error}`,
        );
        continue;
      }
      ...
      for (const chunk of chunks) {
        ...
        // content is shared by reference across this file's chunks.
        items.push({
    ...
    progressCallback?.({
      phase: `Contextual retrieval: submitting ${items.length} chunks to Gemini Batch API...`,
      current: BATCH_WAIT_BAND.start,
      total: 100,
      percentage: BATCH_WAIT_BAND.start,
    });
    console.log(
      `[Context] 🧩 Submitting ${items.length} chunks for batch contextualization...`,
    );

    const contexts = await this.contextualizer.contextualizeBatchViaBatchApi(
      items,
      (p) => {
        // The batch job is opaque (no internal % done), so advance within the
        // wait band by an elapsed-time creep that asymptotes toward — but never
        // reaches — the next job's sub-band, keeping progress visibly moving.
        const elapsedSec = Math.round(p.elapsedMs / 1000);
        const span = BATCH_WAIT_BAND.end - BATCH_WAIT_BAND.start;
        const jobSpan = span / Math.max(1, p.jobTotal);
        const jobBase = BATCH_WAIT_BAND.start + (p.jobNum - 1) * jobSpan;
        const frac = 1 - Math.exp(-p.elapsedMs / BATCH_CREEP_TIME_CONSTANT_MS);
        const pct = Math.round(jobBase + jobSpan * Math.min(0.95, frac));
        progressCallback?.({
          phase: `Contextual retrieval: waiting on Gemini Batch API job ${p.jobNum}/${p.jobTotal} (${p.state}, ${elapsedSec}s elapsed)`,
          current: pct,
          total: 100,
          percentage: pct,
        });
  ...
  private async generateChunkContexts(
    ...
    const items = chunks.map((chunk) => {
      ...
      return {
        chunkContent: chunk.content,
        // Fall back to the chunk itself if the file can't be read.
        fileContent: fileContent || chunk.content,
        relativePath: filePath || "unknown",
      };
    });

    try {
      return await this.contextualizer.contextualizeBatch(items);
    } catch (error) {
      console.warn(
        `[Context] ⚠️  Contextual retrieval failed for batch, falling back to raw chunks:`,
        error,
      );
      return new Array(chunks.length).fill("");
    }
  ...
  private async processChunkBatch(
    ...
    // Optionally generate an LLM context blurb per chunk (contextual retrieval).
    ...
    const embeddings = await this.embedding.embedBatch(embeddingInputs);
  ...
  private async loadIgnorePatterns(
    ...
    try {
      ...
      if (fileBasedPatterns.length > 0 || additionalIgnorePatterns.length > 0) {
        console.log(
          `[Context] 🚫 Loaded total ${fileBasedPatterns.length} ignore patterns from all ignore files and ${additionalIgnorePatterns.length} request ignore patterns`,
        );
      } else {
